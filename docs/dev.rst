...
* set your ``LMS_HOST`` to `local.overhang.io`_ (a conveninece domain that simply `points at 127.0.0.1`_),

* prompt for a platform details (with suitable defaults),

* offer to import the demo course,

* offer create a superuser,

* build an ``openedx-dev`` image, which based ``openedx`` production image but is `specialized for developer usage`_,
...
.. _local.overhang.io: http://local.overhang.io
.. _points at 127.0.0.1: https://dnschecker.org/#A/local.overhang.io
