...
class BasePlugin:
    ...
    def _install(self) -> None:
        ...
        hooks.Actions.LOAD_PLUGIN(self.name).add()(self.__load)
    ...
    def __load(self) -> None:
        ...
        hooks.Filters.PLUGINS_LOADED.add_item(self.name)
    ...
    def _load_tasks(self) -> None:
        ...
        for img, tag in remote_image_tasks.items():
            hooks.Filters.APP_TASK_IMAGES_PULL.add_item(
                (img, tag),
            )
            hooks.Filters.APP_TASK_IMAGES_PUSH.add_item(
                (img, tag),
            )
...
@hooks.Actions.DISCOVER_PLUGINS.add()
def _discover_v0_plugins() -> None:
    """
    Install all entrypoint and dict plugins.

    Plugins from both classes are installed in a contexts, to make it easier to disable
    them in tests.

    Note that official plugins are not installed here. That's because they are expected
    to be installed manually from within the tutor binary.
